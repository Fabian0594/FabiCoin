...
def test_block_hash_deterministic() -> None:
    ...
    data = {"transactions": ["Alice pays Bob 10 BTC"]}
    previous_hash = "genesis_hash"
    nonce = 42

    hash1 = Block.calculate_hash(index, timestamp, data, previous_hash, nonce)
    hash2 = Block.calculate_hash(index, timestamp, data, previous_hash, nonce)
...
def test_block_hash_changes_with_different_inputs() -> None:
    ...
    base_hash = Block.calculate_hash(index, timestamp, data, previous_hash, nonce)

    # Modificar index
    assert base_hash != Block.calculate_hash(
        index + 1, timestamp, data, previous_hash, nonce
    )

    # Modificar timestamp
    assert base_hash != Block.calculate_hash(
        index, timestamp + 1.0, data, previous_hash, nonce
    )

    # Modificar data
    assert base_hash != Block.calculate_hash(
        index, timestamp, "Different Data", previous_hash, nonce
    )

    # Modificar previous_hash
    assert base_hash != Block.calculate_hash(
        index, timestamp, data, "new_previous_hash", nonce
    )

    # Modificar nonce
    assert base_hash != Block.calculate_hash(
        index, timestamp, data, previous_hash, nonce + 1
    )
